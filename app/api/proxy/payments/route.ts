...
import { validateApiKey, proxyCorsOptions } from "@/lib/proxy";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Note: Payments are handled through billing routes, not as a proxy service
    // If you want to enable payment proxy, add "payment" to ProxyService enum in types/proxy.ts

    if (!validation.apiKey) {
      return NextResponse.json({ error: "Invalid API key" }, { status: 401 });
    }

    const projectId = validation.apiKey.project.id;
