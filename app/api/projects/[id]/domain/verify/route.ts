  ...
  try {
    ...
    try {
      ...
      cnameVerified = cnameRecords.some(
        (record) => record.toLowerCase() === "cname.rulxy.com",
      );
      if (!cnameVerified) {
        errors.push(
          `CNAME record found but points to wrong target. Expected: cname.rulxy.com, Got: ${cnameRecords[0]}`,
        );
      }
    } catch (error: unknown) {
      const dnsError = error as { code?: string };
    ...
    if (verified) {
      // Update project as verified using snake_case
